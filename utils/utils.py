...
def postprocess(x, anchors, regression, classification, regressBoxes, clipBoxes, threshold, iou_threshold):
    ...
    # regression : torch.Size([6,46035,4]) anchors(1,46035,4)
    transformed_anchors = clipBoxes(transformed_anchors, x)
    # torch.Size([6,46035,4])
    scores = torch.max(classification, dim=2, keepdim=True)[0] # max_object = [values, indexes]
    # torch.Size([6,46035,1])
    # threshold = 0.25
    scores_over_thresh = (scores > threshold)[:, :, 0]
    # torch.Size([6,46035])
    out = []
    for i in range(x.shape[0]): # x : (6,3,384,640)
